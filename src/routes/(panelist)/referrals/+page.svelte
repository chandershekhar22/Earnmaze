...
<div class="space-y-5 animate-fade-in">
	...
	{#if referralCode}
		<div class="relative overflow-hidden bg-gradient-to-br from-violet-600/15 via-fuchsia-600/10 to-surface-100 border border-violet-500/15 rounded-2xl p-5 md:p-6">
			<div class="absolute -top-12 -end-12 w-32 h-32 bg-violet-500/10 rounded-full blur-3xl"></div>
			<div class="absolute bottom-0 start-0 w-24 h-24 bg-fuchsia-500/8 rounded-full blur-2xl"></div>
			...
			<div class="relative">
				<div class="flex items-center gap-3 mb-4">
					...
					<div>
						<h2 class="text-base font-bold text-white">{m.ref_invite_title()}</h2>
						<p class="text-xs text-neutral-500">{m.ref_invite_desc()}</p>
					</div>
				</div>
				...
				<div class="flex items-center gap-4 text-xs text-neutral-500">
					<span>{m.ref_your_code()} <span class="font-mono font-bold text-white">{referralCode}</span></span>
				</div>
			</div>
		</div>
	{/if}
		...
		<div class="relative overflow-hidden bg-gradient-to-br from-sky-500/[0.08] to-surface-100 rounded-xl p-4 border border-sky-500/10">
			...
			<div class="relative">
				<div class="flex items-center gap-2 mb-2">
					...
					<span class="text-[10px] font-bold text-sky-300/40 uppercase tracking-widest">{m.ref_stat_total()}</span>
				</div>
		...
		<div class="relative overflow-hidden bg-gradient-to-br from-amber-500/[0.08] to-surface-100 rounded-xl p-4 border border-amber-500/10">
			...
			<div class="relative">
				<div class="flex items-center gap-2 mb-2">
					...
					<span class="text-[10px] font-bold text-amber-300/40 uppercase tracking-widest">{m.ref_stat_pending()}</span>
				</div>
		...
		<div class="relative overflow-hidden bg-gradient-to-br from-emerald-500/[0.08] to-surface-100 rounded-xl p-4 border border-emerald-500/10">
			...
			<div class="relative">
				<div class="flex items-center gap-2 mb-2">
					...
					<span class="text-[10px] font-bold text-emerald-300/40 uppercase tracking-widest">{m.ref_stat_paid()}</span>
				</div>
		...
		<div class="relative overflow-hidden bg-gradient-to-br from-violet-500/[0.08] to-surface-100 rounded-xl p-4 border border-violet-500/10">
			...
			<div class="relative">
				<div class="flex items-center gap-2 mb-2">
					...
					<span class="text-[10px] font-bold text-violet-300/40 uppercase tracking-widest">{m.ref_stat_earned()}</span>
				</div>
				<div class="text-xl font-black text-white">{stats.totalEarned.toLocaleString()}</div>
				<div class="text-[10px] text-violet-400/50 font-medium mt-0.5">{m.dash_pts_short()}</div>
			</div>
		</div>
	</div>
	...
	<div class="grid grid-cols-1 md:grid-cols-3 gap-2.5">
		<div class="bg-surface-100 border border-white/[0.06] rounded-xl p-4 flex items-start gap-3">
			...
			<div>
				<div class="text-sm font-bold text-white">{m.ref_step1_title()}</div>
				<div class="text-xs text-neutral-500 mt-0.5">{m.ref_step1_desc()}</div>
			</div>
		</div>
		<div class="bg-surface-100 border border-white/[0.06] rounded-xl p-4 flex items-start gap-3">
			...
			<div>
				<div class="text-sm font-bold text-white">{m.ref_step2_title()}</div>
				<div class="text-xs text-neutral-500 mt-0.5">{m.ref_step2_desc()}</div>
			</div>
		</div>
		<div class="bg-surface-100 border border-white/[0.06] rounded-xl p-4 flex items-start gap-3">
			...
			<div>
				<div class="text-sm font-bold text-white">{m.ref_step3_title()}</div>
				<div class="text-xs text-neutral-500 mt-0.5">{m.ref_step3_desc()}</div>
			</div>
		</div>
	</div>
	...
	<!-- Referrals List -->
	<div>
		<h2 class="text-xs font-bold text-white/40 uppercase tracking-widest mb-3">{m.ref_your_referrals()}</h2>
		...
		{#if referrals.length === 0}
			<div class="bg-surface-100 border border-white/[0.06] rounded-2xl text-center py-12 px-6">
				...
				<p class="text-sm font-semibold text-white/30 mb-1">{m.ref_empty_title()}</p>
				<p class="text-xs text-neutral-600 mb-4">{m.ref_empty_desc()}</p>
				{#if referralLink}
					<button onclick={copyLink} class="btn-primary !text-xs">
						<Copy class="w-3.5 h-3.5" /> {m.ref_copy_link_button()}
					</button>
				{/if}
			</div>
		{:else}
					...
					<div class="bg-surface-100 border border-white/[0.06] rounded-xl px-4 py-3 flex items-center gap-3">
						...
						<div class="flex-1 min-w-0">
							...
							<div class="text-[10px] text-neutral-600 mt-0.5">
								{ref.referredEmail} &middot; {m.ref_joined()} {relDate(ref.createdAt)}
								{#if ref.qualifiedAt}
									&middot; {m.ref_qualified_at()} {relDate(ref.qualifiedAt)}
								{/if}
							</div>
						</div>
						...
						<div class="text-end flex-shrink-0">
							{#if ref.status === 'completed' || ref.status === 'paid'}
								...
								<div class="text-[10px] text-emerald-400/50">{m.ref_pts_earned()}</div>
							{:else if ref.status === 'qualified'}
								<div class="text-sm font-black text-sky-400">+{ref.bonus}</div>
								<div class="text-[10px] text-sky-400/50">{m.ref_processing()}</div>
							{:else}
								<div class="text-sm font-bold text-neutral-600">--</div>
								<div class="text-[10px] text-neutral-700">{m.ref_awaiting()}</div>
							{/if}
						</div>
					</div>
				{/each}
