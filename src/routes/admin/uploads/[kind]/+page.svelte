<script lang="ts">
	...
	async function submit(e: Event) {
		...
		uploading = true;
		try {
			const res = await fetch(`/api/uploads/${data.kind}`, { method: 'POST', body: fd });
			if (!res.ok) {
				const j = await res.json().catch(() => ({}));
				throw new Error(j.message || j.error || `Upload failed (${res.status})`);
			}
			ok = `${data.kindLabel} item uploaded — live now.`;
			title = '';
			desc = '';
			file = null;
			setThumb(null);
...
<div class="max-w-5xl mx-auto px-4 py-8 space-y-8">
	<div class="flex items-center gap-3">
		...
		<div>
			<h1 class="text-xl font-bold text-white">{data.kindLabel}</h1>
			<p class="text-xs text-neutral-400">Upload HTML files. They go live on /{data.kind} immediately.</p>
		</div>
	</div>

	<form onsubmit={submit} class="bg-surface-100 border border-white/[0.06] rounded-xl p-5 space-y-4">
		<h2 class="text-sm font-bold text-white">Upload new {data.kindLabel.toLowerCase()} item</h2>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<label class="block">
				...
				<input bind:value={title} required type="text" placeholder="Title"
		...
		<label class="block">
			...
			<textarea bind:value={desc} required rows="2" placeholder="Short description shown on the card."
		...
		<div>
			...
			>
				...
				{:else}
					...
					<div class="text-xs text-neutral-500 mt-1">PNG, JPG, WEBP, GIF, SVG · max 2 MB · used as the card cover</div>
				{/if}
			</label>
		</div>
		...
		<div class="flex justify-end">
			<button type="submit" disabled={uploading || !title || !desc || !file}
				...
				{#if uploading}<Loader2 class="w-4 h-4 animate-spin" /> Uploading…{:else}<Upload class="w-4 h-4" /> Upload{/if}
			</button>
		</div>
	</form>
	...
	<section>
		...
		{#if data.items.length === 0}
			<div class="bg-surface-100 border border-white/[0.06] rounded-xl p-8 text-center text-sm text-neutral-400">
				No items yet. Upload your first .html file above.
			</div>
		{:else}
