<script lang="ts">
  ...
  const ARTIFACTS = $derived(data.items as any[]);

  const FILTERS = $derived([
    { cat: 'all', label: 'All', count: ARTIFACTS.length },
    { cat: 'trending', label: 'Trending', count: ARTIFACTS.filter(a => a.tags?.includes('TRENDING')).length },
    { cat: 'new', label: 'New', count: ARTIFACTS.filter(a => a.tags?.includes('NEW')).length },
    { cat: 'data', label: 'Data', count: ARTIFACTS.filter(a => a.cat === 'data').length },
    { cat: 'lifestyle', label: 'Lifestyle', count: ARTIFACTS.filter(a => a.cat === 'lifestyle').length },
  ]);

  let filtered = $derived(
    activeFilter === 'all' ? ARTIFACTS :
    activeFilter === 'trending' ? ARTIFACTS.filter(a => a.tags?.includes('TRENDING')) :
    activeFilter === 'new' ? ARTIFACTS.filter(a => a.tags?.includes('NEW')) :
    ARTIFACTS.filter(a => a.cat === activeFilter)
  );
</script>
...
<nav class="a-nav">
  <div class="container">
    ...
    <div class="nav-links">
      ...
      <a href="/artifacts" class="active">Artifacts</a>
    </div>
...
<section class="hero">
  <div class="container">
    ...
    <h1>Explore <span class="grad-text">Interactive</span> Experiences</h1>
    <p>Hand-picked interactive artifacts you can explore, play with, and learn from — right inside EarnMaze.</p>
  </div>
</section>
    ...
    <div class="a-grid">
      ...
      {:else}
        {#each filtered as a (a.id)}
          <a class="a-card" href="/artifacts/{a.id}" data-sveltekit-reload>
