<script lang="ts">
	...
	let { data } = $props<{ data: { artifact: any; stats: { likes: number; shares: number } } }>();
	const a = data.artifact;
	const stats = data.stats ?? { likes: 0, shares: 0 };
</script>

<svelte:head>
	<title>{a.title} — EarnMaze Artifacts</title>
</svelte:head>
...
<div class="viewer">
	<div class="v-bar">
		<div class="v-l">
			<a href="/artifacts" data-sveltekit-reload class="v-back">← Back</a>
			<div class="v-meta">
		...
		<div class="v-r">
			<SocialButtons kind="artifacts" id={a.id} title={a.title} likes={stats.likes} shares={stats.shares} variant="bar" />
			<div class="v-tags">
				<span class="v-tag {a.cat}">{a.cat}</span>
				{#each (a.tags || []) as t}
	...
	<!-- Served via /artifact-content/<id> so the persistence layer is injected.
	     allow-same-origin is required for the injected script to use localStorage
	     (see $lib/server/html-inject for the security tradeoff). The
	     use:persistState action is the DOM-restore fallback for artifacts that
	     never call saveState. -->
	<iframe src={`/artifact-content/${a.id}`} title={a.title} sandbox="allow-scripts allow-same-origin allow-forms allow-popups" use:persistState={`artifacts:${a.id}`}></iframe>
</div>
