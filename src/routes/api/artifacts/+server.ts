...
const BG_BY_CAT: Record<ArtifactCat, string> = {
...
const ICON_BY_CAT: Record<ArtifactCat, string> = {
	data: 'chart',
	lifestyle: 'fashion',
	other: 'bars',
};
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const cat: ArtifactCat = catRaw === 'data' || catRaw === 'lifestyle' ? catRaw : 'other';
	...
	if (thumbFile && thumbFile instanceof File && thumbFile.size > 0) {
		...
		if (thumbFile.size > 2 * 1024 * 1024)
			throw error(400, 'Thumbnail too large (max 2 MB)');
	}

	const id = `${slugify(title)}-${Date.now().toString(36)}`;
	const html = await file.text();
	await saveArtifactHtml(id, html);

	let thumbUrl: string | undefined;
	if (thumbFile && thumbExt) {
		const buf = new Uint8Array(await thumbFile.arrayBuffer());
		thumbUrl = await saveArtifactThumb(id, buf, thumbExt);
	}

	const tags: ArtifactTag[] = [];
	...
	await addArtifact({
		...
		file: `/artifacts-uploaded/${id}/index.html`,
		thumb: thumbUrl,
	});
