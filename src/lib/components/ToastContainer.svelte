	...
	<div class="fixed top-4 right-4 z-50 space-y-2 max-w-sm">
		{#each toastStore.toasts as toast (toast.id)}
			<div
				...
				aria-live="assertive"
			>
				<div class="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold mr-3 {toast.type === 'success' ? 'bg-emerald-500/10 text-emerald-400' : toast.type === 'error' ? 'bg-rose-500/10 text-rose-400' : toast.type === 'warning' ? 'bg-amber-500/10 text-amber-400' : 'bg-primary-500/10 text-primary-400'}">
					{getIcon(toast.type)}
				</div>
				...
				<button
					...
					class="flex-shrink-0 ml-3 w-5 h-5 rounded-full flex items-center justify-center hover:bg-white/10 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-surface-100 focus:ring-neutral-500 text-neutral-500 hover:text-neutral-300"
